...
public class HBaseOutputFormat implements OutputFormat<Text, Text>,
	...
	private boolean put(Put p, String[] args) {
		if (omitcf) {
			if (args.length > 3) {
				p.addColumn(defaultCF.getBytes(), args[1].getBytes(),
						getTimestampString(args[3]), args[2].getBytes());
			...
			if (args.length > 4) {
				p.addColumn(args[1].getBytes(), args[2].getBytes(),
						getTimestampString(args[4]), args[3].getBytes());
	...
	private class HBaseRecordWriter implements RecordWriter<Text, Text> {
		...
		public void write(Text key, Text value) throws IOException {
			Put put = createUpdate(key.toString(), value.toString());
			if (put != null) {
